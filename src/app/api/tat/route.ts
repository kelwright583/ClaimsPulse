...
export async function GET() {
  try {
    ...
    const [breaches, tatConfigs, activeDelays] = await Promise.all([
    ...
    for (const s of breaches) {
      ...
      const tat = s.secondaryStatus ? tatMap.get(s.secondaryStatus) : null;
      const priority = tat?.priority ?? 'standard';

      if (!grouped[status]) {
        grouped[status] = {
          priority,
          maxDays: tat?.maxDays ?? 0,
          claims: [],
        };
      }
