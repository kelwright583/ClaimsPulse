import * as XLSX from 'xlsx';
import type { BrokerPerformanceReportData } from './broker-performance-data';

export function generateBrokerPerformanceXlsx(data: BrokerPerformanceReportData): Uint8Array {
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([
    ['Broker Performance Report'],
    ['Snapshot Date', data.snapshotDate],
    ['Date Range', data.dateRange],
  ]), 'Summary');
  XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(data.brokers), 'PerBroker');
  return XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' }) as Uint8Array;
}
